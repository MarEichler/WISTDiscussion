...
**If you simply clone the resposirory on your local machine: all of these are also included in the `data/` folder, but not the `build_vio_nhood_data.csv`, as it is extremely big.** You can use as many or as few of these as you would like, and also supplement with any additional information you can find and cite.
...
<span style="color: red;">**Remember, the PRIMARY and more substantial questions of interest will be released on 3/5 at the Data Dive. Your job before then is to simply consider the [exploratory questions](#exploratory-questions).**  </span>

## Exploratory Questions

* Are fires the most common incident? What other incidents happen within Chicago? 
* How often is the Red Cross responding to incidents? How do these patterns change over time? Do you see evidence for a spatial pattern?
* How much financial assistance is given each day? How do these patterns change over time? Do you see evidence for a spatial pattern?
...
Your team will need to create 1 slides (keep it simple) with your best visualization or takeaway. You will get a maximum 90 seconds to present your slide at [WiDS downtown](widschicago.org) on Friday, March 6!

Please email your 1 slide with the email subject: "DATA DIVE", and a list of team members' names that will be attending WiDS: downtown to <mcwhalen@u.northwestern.edu> by 8:00 pm. 


# The Data

The Red Cross of Greater Chicago/Illinois responds to disaster incidents throughout Illinois and provides assistance to those who are affected by those disasters.

The dataset provided by the Red Cross has each row as an incident with the different columns as variables for that incident. The Red Cross mostly responds to fires disasters and a bulk of the disasters happening the city of Chicago, the data provided is only from Chicago. 

The [US Fire Administration](https://www.usfa.fema.gov/data/statistics/) has collected data on fires from a variety of sources. While these are national estimates and do not adequately represent Chicago-specific fires, it can further our nderstanding on fire disasters. Data is given about buildings in Chicago as a possible way to look at they why/how these disasters, primarily fires, are occurring. We provide data on building codes violations, environmental factors, vacant buildings, and 311 calls data to aid with exploratory analysis. 
