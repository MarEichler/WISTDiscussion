...
You will have **2.5 hours** to complete the following task. 

Here are some potetial packages that might be of use in your exploratory data analysis.
...
* Shapefile for community areas in Chicago (**NOTE**: you need ALL of the file types in that `neighborhood_shapefile/` directory, not just the *.shp!)
* [Community Snapshot Data](https://datahub.cmap.illinois.gov/dataset/1d2dd970-f0a6-4736-96a1-3caeb431f5e4/resource/d23fc5b1-0bb5-4bcc-bf70-688201534833/download/CDSFieldDescriptions201906.pdf) by neighborhood. This has a TON of information you can use: population, demographics, and transit information!
...
Your team will need to create 1 slides (keep it simple) with your best visualization or takeaway. You will get a maximum 90 seconds to present your slide at [WiDS downtown](widschicago.org) tomorrow!
...
Our incident data has as a zip code variable. The below example code joins Red Cross incident data with spatial data based on zip code, and makes a map of the number of incidents in 2019. 
...
il_map_dat <- readOGR(dsn="neighborhood_shapefiles/Neighborhoods_2012b.shp")
...
## Adding Community Area to Environment and 311 Call Data

```{r}
library(sf)
nhood <- st_read(dsn="neighborhood_shapefiles/Neighborhoods_2012b.shp")

enviro <- read_csv('enviro_inspection.csv' ) %>% 
  drop_na(longitude)

enviro <-  st_as_sf(enviro, coords = c("longitude", "latitude"))

#enviro <- st_transform(enviro, "+proj=longlat  +datum=WGS84")
nhood <- st_transform(nhood, "+proj=longlat  +datum=WGS84")

st_crs(enviro) <- st_crs(nhood)
# this shows which neighbourhoods (HEITI) intersects each school
enviro_nhood_data <- st_join(enviro, nhood['PRI_NEIGH'], join = st_intersects)

calls_311 <- read_csv('calls_311.csv' ) %>% 
  drop_na(longitude)
